...
LASSO_RESULTS = ensure_dir(TABLES / "lasso_results")
LASSO_CACHE = ensure_dir(CACHED_RESULTS / "lasso_selection")
...
def lasso_select_target(
    ...
    estimator: LassoCV = cast(LassoCV, RegressionModel.Lasso.get())
    results = DataFrame(
        cross_validate(
            estimator,
            X_train,
    ...
    y_pred_test: ndarray = estimator.predict(X_test)  # type: ignore
    y_pred_train: ndarray = estimator.predict(X_train)  # type: ignore
...
def nested_lasso_feature_select(
    ...
    all_fold_out = LASSO_RESULTS / f"{fbase}_nested.parquet"
    ...
    for target in pbar:
        ...
        fold_out = LASSO_CACHE / f"{fbase}_{tname}_folds.parquet"
